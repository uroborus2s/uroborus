...
export const createGetCssVar = (cssVarPrefix = 'uro') =>
...
export default function extendTheme(themeOptions?: CssVarsThemeOptions): Theme {
   ...
   */
  function attachColorChannels(palette: Palette) {
    (
      Object.entries(palette) as Array<[ColorPaletteProp, PaletteRange]>
    ).forEach(([, paletteRange]) => {
      ...
      if (typeof paletteRange === 'object') {
        if (!paletteRange.mainChannel && paletteRange[channelMapping.main]) {
          paletteRange.mainChannel = colorChannel(
            paletteRange[channelMapping.main],
          );
        }
        if (!paletteRange.lightChannel && paletteRange[channelMapping.light]) {
          paletteRange.lightChannel = colorChannel(
            paletteRange[channelMapping.light],
          );
        }
        if (!paletteRange.darkChannel && paletteRange[channelMapping.dark]) {
          paletteRange.darkChannel = colorChannel(
            paletteRange[channelMapping.dark],
          );
        }
      }
    });
