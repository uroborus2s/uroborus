import { IconColorType, IconNames } from '@/util';
...
export interface NameEntity extends Entity {
  ...
  set(name: string, desc: string): void;
}

export type PermissionEntityProps = Partial<PermissionEntity>;
...
export interface PermissionEntity extends Entity {
  ...
  share_able: boolean;
  update_able: boolean;
  set: (per: PermissionEntityProps) => void;
}

export type LogEntityProps = Partial<Omit<LogEntity, 'set'>>;

export interface LogEntity extends Entity {
  updated_at: string;
  created_at: string;
  joined_at: string;
  set: (value: LogEntityProps) => void;
}

export interface OrderEntity extends Entity {
  order: number;

  set(order: number): void;

  compare(other: number | OrderEntity): number;
}
