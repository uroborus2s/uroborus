const parcelRequire = (function (init) {
   ...
   * linear
   *
   * @param {number} t time: ?在?刻 (0 ~ duration)
   * @param {number} b begin: 檫始位置
   * @param {number} c change: 檫始位置から剿了位置までの?化量 (finish - begin)
   * @param {number} d duration: 全体?殓
   * @returns {number} ?在位置
   */
   ...
   * アニメ┼ション轹数
   *
   * @param {AnimationCallback} fn アニメ┼ションフレ┼ム?に?行するコ┼ルバック
   * @param {Object} [options={}] オプション
   * @param {number} [options.begin=0] 檫始位置
   * @param {number} [options.finish=1] 剿了位置
   * @param {number} [options.duration=500] 全体?殓
   ...
   * @param {number} position ?在位置
   * @param {number} time ?在?刻 (0 ~ duration)
   */
  ...
  function $vFOr$export$getTextCoordinate(option) {
    ...
    document.body.appendChild(canvas); // letter-spacing を有?にするために必要
